...
selected_col <- grep("Gene|^Wizard Acclimated(\\.([1-3]))?$|^Hiverna Acclimated(\\.([1-3]))?$", colnames(count_matrix), value = TRUE)
WA_vs_HA <- count_matrix[, selected_col]
```

```{r}
# Prepare DESeq2 count matrix
counts_matrix_WAvsHA <- WA_vs_HA
rownames(counts_matrix_WAvsHA) <- counts_matrix_WAvsHA$Gene
counts_matrix_WAvsHA <- counts_matrix_WAvsHA[, -1]  # remove Gene column

```

```{r}
sample_names <- colnames(counts_matrix_WAvsHA)

# Define conditions in the correct order
conditions <- rep(c("Wizard Acclimated", "Hiverna Acclimated"), each = 3)
...
dds <- DESeqDataSetFromMatrix(countData = counts_matrix_WAvsHA,
                              colData = metadata,
...
res <- results(dds, contrast = c("Condition", "Wizard Acclimated", "Hiverna Acclimated"))
...
write.csv(res_final, "rankedlist_WAvsHA.csv", row.names = FALSE)
```
...
volcano_plot <- ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = Expression)) +
    ...
    ggtitle('Volcano Plot: Wizard Acclimated vs. Hiverna Acclimated')+
    theme(
...
ggsave("volcano_plot_WAvsHA.png", plot = volcano_plot, width = 8, height = 7, dpi = 300)
```
