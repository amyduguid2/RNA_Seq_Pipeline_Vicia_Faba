...
selected_col <- grep("Gene|Hiverna Control|^Hiverna Acclimated(\\.([1-3]))?$", colnames(count_matrix), value = TRUE)
HCvsHA <- count_matrix[, selected_col]
```

```{r}
# Prepare DESeq2 count matrix
counts_matrix_HCvsHA <- HCvsHA
rownames(counts_matrix_HCvsHA) <- counts_matrix_HCvsHA$Gene
counts_matrix_HCvsHA <- counts_matrix_HCvsHA[, -1]  # remove Gene column

```

```{r}
sample_names <- colnames(counts_matrix_HCvsHA)

# Define conditions in the correct order
conditions <- rep(c("Hiverna Control", "Hiverna Acclimated"), each = 3)
...
dds <- DESeqDataSetFromMatrix(countData = counts_matrix_HCvsHA,
                              colData = metadata,
...
# Extract results 
# pos log fold change is upregulated in Acclimated
# neg log fold change is upregulated in Control
res <- results(dds, contrast = c("Condition", "Hiverna Acclimated", "Hiverna Control"))
...
write.csv(res_final, "rankedlist_HCvsHA.csv", row.names = FALSE)
```
...
volcano_plot <- ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = Expression)) +
    ...
    ggtitle('Volcano Plot: Hiverna Control vs. Hiverna Acclimated')+
    theme(
...
ggsave("volcano_plot_HCvsHA.png", plot = volcano_plot, width = 8, height = 7, dpi = 300)
```