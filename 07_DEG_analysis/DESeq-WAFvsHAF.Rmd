...
selected_col <- grep("Gene|Wizard Acclimated & Freezing|Hiverna Acclimated & Freezing", colnames(count_matrix), value = TRUE)
WAF_vs_HAF <- count_matrix[, selected_col]
```

```{r}
# Prepare DESeq2 count matrix
counts_matrix_WAF_vs_HAF <- WAF_vs_HAF
rownames(counts_matrix_WAF_vs_HAF) <- counts_matrix_WAF_vs_HAF$Gene
counts_matrix_WAF_vs_HAF <- counts_matrix_WAF_vs_HAF[, -1]  # remove Gene column

```

```{r}
sample_names <- colnames(counts_matrix_WAF_vs_HAF)

# Define conditions in the correct order
conditions <- rep(c("Wizard Acclimated & Freezing", "Hiverna Acclimated & Freezing"), each = 3)
...
dds <- DESeqDataSetFromMatrix(countData = counts_matrix_WAF_vs_HAF,
                              colData = metadata,
...
# Extract results 
# pos log fold change is upregulated in Wizard
# neg log fold change is upregulated in Hiverna
res <- results(dds, contrast = c("Condition", "Wizard Acclimated & Freezing", "Hiverna Acclimated & Freezing"))
...
write.csv(res_final, "rankedlist_WAFvsHAF.csv", row.names = FALSE)
```
...
res$Expression[res$padj < 0.001 & res$log2FoldChange > 1] <- "Upregulated" #upregulated genes
res$Expression[res$padj < 0.001 & res$log2FoldChange < -1] <- "Downregulated" #downregulated genes
...
volcano_plot <- ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = Expression)) +
    geom_vline(xintercept = c(-1,1), col = "gray", linetype = "dashed")+ #log2FC threshold
    ...
    ggtitle('Volcano Plot: Wizard Acclimated Freezing 
             vs. Hiverna Acclimated Freezing')+
    theme(
...
ggsave("volcano_plot_WAFvsHAF.png", plot = volcano_plot, width = 8, height = 7, dpi = 300)
```
