...
selected_col <- grep("Gene|Hiverna & Freezing|Hiverna Acclimated & Freezing", colnames(count_matrix), value = TRUE)
HFvsHAF <- count_matrix[, selected_col]
```

```{r}
# Prepare DESeq2 count matrix
counts_matrix_HFvsHAF <- HFvsHAF
rownames(counts_matrix_HFvsHAF) <- counts_matrix_HFvsHAF$Gene
counts_matrix_HFvsHAF <- counts_matrix_HFvsHAF[, -1]  # remove Gene column

```

```{r}
sample_names <- colnames(counts_matrix_HFvsHAF)

# Define conditions in the correct order
conditions <- rep(c("Hiverna Freezing", "Hiverna Acclimated Freezing"), each = 3)
...
dds <- DESeqDataSetFromMatrix(countData = counts_matrix_HFvsHAF,
                              colData = metadata,
...
# neg log fold change is upregulated in Freezing
res <- results(dds, contrast = c("Condition", "Hiverna Acclimated Freezing", "Hiverna Freezing"))
...
write.csv(res_final, "rankedlist_HFvsHAF.csv", row.names = FALSE)
```
...
volcano_plot <- ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = Expression)) +
    ...
    ggtitle('Volcano Plot: Hiverna Freezing vs. Hiverna Acclimated Freezing')+
    theme(
...
ggsave("volcano_plot_HFvsHAF.png", plot = volcano_plot, width = 8, height = 7, dpi = 300)
```