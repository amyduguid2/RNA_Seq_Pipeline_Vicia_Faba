...
selected_col <- grep("Gene|Wizard Control|^Wizard Acclimated(\\.([1-3]))?$", colnames(count_matrix), value = TRUE)
WCvsWA <- count_matrix[, selected_col]
```

```{r}
# Prepare DESeq2 count matrix
counts_matrix_WCvsWA <- WCvsWA
rownames(counts_matrix_WCvsWA) <- counts_matrix_WCvsWA$Gene
counts_matrix_WCvsWA <- counts_matrix_WCvsWA[, -1]  # remove Gene column

```

```{r}
sample_names <- colnames(counts_matrix_WCvsWA)

# Define conditions in the correct order
conditions <- rep(c("Wizard Control", "Wizard Acclimated"), each = 3)
...
dds <- DESeqDataSetFromMatrix(countData = counts_matrix_WCvsWA,
                              colData = metadata,
...
res <- results(dds, contrast = c("Condition", "Wizard Acclimated", "Wizard Control"))
...
write.csv(res_final, "rankedlist_WCvsWA.csv", row.names = FALSE)
```
...
volcano_plot <- ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = Expression)) +
    ...
    ggtitle('Volcano Plot: Wizard Control vs. Wizard Acclimated')+
    theme(
...
ggsave("volcano_plot_WCvsWA.png", plot = volcano_plot, width = 8, height = 7, dpi = 300)
```