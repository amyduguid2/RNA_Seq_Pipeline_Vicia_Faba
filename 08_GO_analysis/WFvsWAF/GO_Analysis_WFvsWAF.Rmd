...
WFvsWAF <- read.csv("/rds/projects/f/foyerch-rnaseq-data-faba-bean/DEG_analysis/rankedlist_WFvsWAF.csv", header = TRUE)
```
...
```{r}
WFvsWAF_upregulated <- WFvsWAF %>% filter(WFvsWAF$log2FoldChange > 1)
```

```{r}
WFvsWAF_downregulated <- WFvsWAF %>% filter(WFvsWAF$log2FoldChange < -1)
```

```{r}
write.table(WFvsWAF_upregulated$gene, file = "upregulated_list_WFvsWAF.txt", 
            quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
write.table(WFvsWAF_downregulated$gene, file = "downregulated_list_WFvsWAF.txt", 
            quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
upregulated_orthologs <- read.csv("/rds/projects/f/foyerch-rnaseq-data-faba-bean/GO_analysis/WFvsWAF/mart_export.txt")
```

```{r}
write.table(upregulated_orthologs$Glycine.max.gene.stable.ID, file = "/rds/projects/f/foyerch-rnaseq-data-faba-bean/GO_analysis/WFvsWAF/upregulated_orthologs_id.txt", 
            quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
GO_upregulated <- read.delim("/rds/projects/f/foyerch-rnaseq-data-faba-bean/GO_analysis/WFvsWAF/analysis.txt", header = TRUE, sep = "\t", quote = "", stringsAsFactors = FALSE)
```
...
```{r}
downregulated_orthologs <- read.csv("/rds/projects/f/foyerch-rnaseq-data-faba-bean/GO_analysis/WFvsWAF/mart_export_down.txt")
```

```{r}
write.table(downregulated_orthologs$Glycine.max.gene.stable.ID, file = "/rds/projects/f/foyerch-rnaseq-data-faba-bean/GO_analysis/WFvsWAF/downregulated_orthologs_id.txt", 
            quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
GO_downregulated <- read.table("/rds/projects/f/foyerch-rnaseq-data-faba-bean/GO_analysis/WFvsWAF/analysis_down.txt", header = TRUE, sep = "\t")
```
...
ggsave("GO_dotplot_WFvsWAF.png", width = 8, height = 5, dpi = 300)
```